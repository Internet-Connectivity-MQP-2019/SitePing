...
const prettifyCity = city => {
	return city.replace(/\b\w/g, function (l) {
		return l.toUpperCase()
	})   // Replace first letters of all words with uppercase letters. \b finds the boundary characters (first letters) of all words found by \w
...
const setupMap = function (width, height) {
	...
	document.querySelector("#view_mobile").onclick = () => {
		setMobile(true)
	};
	document.querySelector("#view_non_mobile").onclick = () => {
		setMobile(false)
	};

	document.querySelector("#view_by_state").onclick = () => {
		setStateView(true)
	};
	document.querySelector("#view_by_city").onclick = () => {
		setStateView(false)
	};
...
const toggleMapVisibility = function() {
	if (stateView === true) {
...
const updateRTTLeaderBoards = () => {
	...
	const updateListWithData = (id, data) => {
		const list = document.querySelector(id);
		list.innerHTML = "";
		data.forEach(item => {
			const city =  prettifyCity(item["city"]);
			list.innerHTML += `<li>${city}, ${item["_id"]["state"]} (${Math.round(item["avg_rtt"])} ms)</li>`;
		});
	};

	const count = 5;

	const top_mobile = data.filter(filterIsMobile)
		.sort(sortRTTDescending)
		.splice(0, count);
	const top_non_mobile = data.filter(filterIsNotMobile)
		.sort(sortRTTDescending)
		.splice(0, count);
	const worst_mobile = data.filter(filterIsMobile)
		.sort(sortRTTAscending)
		.splice(0, count);
	const worst_non_mobile = data.filter(filterIsNotMobile)
		.sort(sortRTTAscending)
		.splice(0, count);

	updateListWithData('#cityMobileRTTLeaderboard', top_mobile);
	updateListWithData("#cityNonMobileRTTLeaderboard", top_non_mobile);
	updateListWithData("#cityMobileRTTLoserboard", worst_mobile);
	updateListWithData("#cityNonMobileRTTLoserboard", worst_non_mobile);
};
...
const updateMap = function () {
	...
	if (stateView === true) {
		...
		const scaledGradient = makeScaleGradient(stateData);

		updatesStateMap(div, finalStateDataDict, scaledGradient);
	} else {
		const scaledGradient = makeScaleGradient(filteredMobile);
...
const updateTopCitiesByIP = function (rankings) {
	const topCitiesByIPList = document.querySelector("#cityIPLeaderboard");
	topCitiesByIPList.innerHTML = "";
	rankings.forEach(item => {
		const city = prettifyCity(item["_id"]["city"]);
		topCitiesByIPList.innerHTML += `<li>${city}, ${item["_id"]["state"]} (${item["count"]})</li>`;
	});
};
...
const updateTopStatesByIP = function (rankings) {
	const stopStatesByIPList = document.querySelector("#stateIPLeaderboard");
	stopStatesByIPList.innerHTML = "";
	rankings.forEach(item => {
		stopStatesByIPList.innerHTML += `<li>${stateLookup[item["_id"]["state"]]} (${item["count"]})</li>`
	});
};
